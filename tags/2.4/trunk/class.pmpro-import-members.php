...
use E20R\Paid_Memberships_Pro\Import_Members\Import\Import_Member;
use E20R\Paid_Memberships_Pro\Import_Members\Import\Import_User;
...
class Import_Members_From_CSV {
	...
	public function load_plugins() {
		...
		add_action( 'plugins_loaded', array( Import_Member::get_instance(), 'load_actions'), 20 );
		
        add_action( 'init', array( self::get_instance(), 'load_i18n' ), 5 );
		add_action( 'init', array( self::get_instance(), 'process_csv' ) );

        add_action( 'admin_menu', array( Page::get_instance(), 'add_admin_pages' ) );
        add_action( 'admin_notices', array( Error_Log::get_instance(), 'display_admin_message' ), 10 );
        
		add_action( 'admin_enqueue_scripts', array( self::get_instance(), 'admin_enqueue_scripts' ) );
		add_action( 'wp_ajax_import_members_from_csv', array( Ajax::get_instance(), 'wp_ajax_import_from_csv' ) );
		
		// PMPro specific import functionality
		// add_action( 'e20r-pre-member-import', array( self::get_instance(), 'pre_member_import' ) , 10, 2);
	...
	public function process_csv() {
		...
		if ( isset( $_REQUEST['pmp-im-import-members-wpnonce'] ) && ! ( defined('DOING_AJAX' ) && DOING_AJAX !== true ) ) {
            ...
            $error_log->debug("Processing AJAX request");
			...
			if ( isset( $_FILES['members_csv']['tmp_name'] ) ) {
				...
				if ( true === $this->background_import ) {
					...
					if ( empty( $this->filename ) ) {
					    ...
					    $error_log->add_error_msg( __( 'CSV file not selected!', 'import-members-from-csv' ), 'error' );
	...
	public function import_csv( $filename, $args ) {
		...
		while (! feof($fh) ) {
			...
			$userdata = apply_filters( 'e20r-import-userdata', $usermeta, $userdata, $settings );
			...
			$usermeta = apply_filters( 'e20r-import-usermeta', $usermeta, $userdata, $settings );
			...
			if ( is_wp_error( $user_id ) ) {
				...
				do_action( 'e20r-import-post-user-import', $user_id, $settings );
