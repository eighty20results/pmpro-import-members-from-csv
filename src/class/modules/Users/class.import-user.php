...
use E20R\Paid_Memberships_Pro\Import_Members\Validate\Create_Password;
use E20R\Paid_Memberships_Pro\Import_Members\Validate\User_ID;
use E20R\Paid_Memberships_Pro\Import_Members\Validate\User_Update;
use E20R\Paid_Memberships_Pro\Import_Members\Validate\Validate;
use E20R\Paid_Memberships_Pro\Import_Members\Variables;
use E20R\Paid_Memberships_Pro\Import_Members\Validate\Date_Format;
...
class Import_User {
	...
	public function import( $user_data, $user_meta, $headers ) {
		...
		do_action( 'e20r-pre-user-import', $user_data, $user_meta );
		...
		if ( ! in_array( 'user_login', $headers ) &&
		     empty( $user ) && ( ! empty( $user_data['user_email'] ) && is_email( $user_data['user_email'] ) )
		) {
			
			$msg = sprintf(
				__(
					'Created user login field for record at row %d',
					Import_Members_From_CSV::plugin_slug ), $active_line_number );
		...
		if ( empty( $user_data['user_email'] ) || ( ! empty( $user_data['user_email'] ) && ! is_email( $user_data['user_email'] ) ) ) {
			...
			$msg = sprintf(
				__( "Invalid email in row %d (Not imported).", Import_Members_From_CSV::plugin_slug ),
				( $active_line_number )
			);
		...
		if ( ( true === $needs_update && false === $allow_update ) &&
		     true !== ( $error_column = User_Update::validate( $user_data, $allow_update ) )
		) {
		...
		} else if ( false === $needs_update && false === $password_hashing_disabled ) {
			$user_id = wp_insert_user( $user_data );
		} else if ( true === $password_hashing_disabled ) {
			$user_id = self::insert_disabled_hashing_user( $user_data );
		} else {
			$active_line_number ++;
			
			$e20r_import_err["user_not_imported_{$active_line_number}"] = new \WP_Error( 'e20r_im_account', sprintf( __( 'No update/insert action taken for %s', Import_Members_From_CSV::plugin_slug ), $user_data['user_email'] ) );
			$active_line_number;
		...
		$default_role = apply_filters( 'e20r-import-default-user-role', $default_role, $user_id, $site_id );
		...
		if ( is_wp_error( $user_id ) ) {
			...
			if ( ! empty( $user_data['user_registered'] ) &&
			     true === Time::validate( $user_data['user_registered'] )
			) {
			...
			do_action( 'e20r-after-user-import', $user_id, $settings );
	...
	public function insert_disabled_hashing_user( $userdata ) {
		...
		if ( empty( $user_login ) ) {
			return new \WP_Error( 'empty_user_login', __( 'Cannot create a user with an empty login name.', Import_Members_From_CSV::plugin_slug ) );
		}
		if ( false === $update && username_exists( $user_login ) ) {
			return new \WP_Error( 'existing_user_login', sprintf( __( 'Sorry, that username (%s) already exists!', Import_Members_From_CSV::plugin_slug ), $user_login ) );
		}
		...
		if ( false === $update && ! defined( 'WP_IMPORTING' ) && email_exists( $user_email ) ) {
			return new \WP_Error( 'existing_user_email', sprintf( __( 'Sorry, that email address (%s) is already used!', Import_Members_From_CSV::plugin_slug ), $user_email ) );
		}
		...
		if ( empty( $userdata['display_name'] ) ) {
			if ( true === $update ) {
				...
				$display_name = sprintf( _x( '%1$s %2$s', 'Display name based on first name and last name', Import_Members_From_CSV::plugin_slug ), $meta['first_name'], $meta['last_name'] );
	...
	public function import_usermeta( $user_meta, $user_data ) {
		...
		foreach ( $user_meta as $key => $value ) {
			...
			if ( in_array( $key, array_keys( $variables->get( 'fields' ) ) ) ) {
				$key = "imported_{$key}";
			}
