...
class Sponsor {
	...
	public function __construct( $user_id = null ) {
		...
		if ( !empty( $user_id ) && empty( $this->user ) ) {
			throw new \Exception( sprintf( __( 'No user with that ID (%s) found!', 'pmpro-import-members-from-csv' ), $user_id ) );
		}
	}
	...
	public function set( $param, $value, $type = null ) {
		...
		if ( ! in_array( $param, $attributes ) && ! in_array( $type, $attributes ) ) {
			return false;
		}
