...
if ( ! class_exists( '\E20R\Import_Members\Error_log' ) ) {
	/**
	 * Class Error_Log
		...
		public function display_admin_message() {
			...
			if ( ! empty( $error_msgs ) && is_admin() ) {
				foreach ( $error_msgs as $msg_info ) {
					?>
					<div class="notice notice-<?php echo esc_attr( $msg_info['type'] ); ?> is-dismissible">
						<p><strong><?php echo esc_attr( $msg_info['message'] ); ?></strong></p>
						<button type="button" class="notice-dismiss">
							<span class="screen-reader-text"><?php esc_attr_e( 'Dismiss this message.', 'pmpro-import-members-from-csv' ); ?></span>
						</button>
					</div>
					<?php
				}
