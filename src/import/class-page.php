...
class Page {
	...
	public function import_page() {
		...
		?>
		<div class="wrap">
		<h2><?php _e( 'Import PMPro members from a CSV file', Import_Members::PLUGIN_SLUG ); ?></h2>
					  <?php
				...
				<p><strong><?php _e( 'IMPORTANT:', Import_Members::PLUGIN_SLUG ); ?></strong> 
									 <?php
										printf(
							...
							<fieldset>
								...
								<select id="site_id" name="site_id">
									<?php foreach ( $site_list as $site ) { ?>
										<option
										value="<?php esc_attr_e( $site->blog_id ); ?>" <?php selected( $site->blog_id, $current_blog_id ); ?>><?php esc_html_e( $site->blogname ); ?></option>
																 <?php
									}
									?>
								</select>
