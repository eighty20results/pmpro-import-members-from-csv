...
use E20R\Paid_Memberships_Pro\Import_Members\Modules\PMPro\Import_Sponsors;
		...
		*/
		...
		if ( empty( $filename ) ) {
			wp_send_json_error( array(
				'status'  => - 1,
				'message' => __( "No import file provided!", Import_Members_From_CSV::plugin_slug ),
			) );
			exit();
		}
		...
		if ( ! file_exists( "{$import_dir}/{$filename}" ) ) {
			wp_send_json_error(
				array(
					'status'  => - 1,
					'message' => sprintf(
						__( "File (%s) not found in %s\nIs the directory writable by the web server software?", Import_Members_From_CSV::plugin_slug ),
						$filename,
						$import_dir
					),
				)
		...
		if ( isset( $_REQUEST['import'] ) ) {
			...
			switch ( $_REQUEST['import'] ) {
				case 'file':
					$status = sprintf( '<div class="error"><p><strong>%s</strong></p></div>', __( 'Error during file upload.', Import_Members_From_CSV::plugin_slug ) );
					break;
				case 'data':
					$status = sprintf( '<div class="error"><p><strong>%s</strong></p></div>', __( 'Cannot extract data from uploaded file or no file was uploaded.', Import_Members_From_CSV::plugin_slug ) );
					break;
				case 'success':
					$status = sprintf( '<div class="updated"><p><strong>%s</strong></p></div>', __( 'Member import was successful.', Import_Members_From_CSV::plugin_slug ) );
					break;
				default:
					$status = null;
			}
		...
		if ( empty( $results['user_ids'] ) ) {
			...
			wp_send_json_error( array(
				'status'         => false,
				'message'        => sprintf( __( "Error during import (# of errors: %d):\n%s", Import_Members_From_CSV::plugin_slug ), count( $msgs ), implode( "\n", $msgs ) ),
				'display_errors' => ( ! empty( $display_errors ) ? $display_errors : null ),
			) );
			exit();
		} else {
			...
			$status_msg = sprintf( __( "Imported %s", Import_Members_From_CSV::plugin_slug ), str_pad( '', count( $results['user_ids'] ), '.' ) ) . "\n";
